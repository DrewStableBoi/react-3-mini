...
class App extends Component {
  ...
  updatePrice(priceChange, id) {
    axios.put(`https://joes-autos.herokuapp.com/api/vehicles/${id}/${priceChange}`).then((results) => {
      toast.success("You've successfully upped the price!");
        this.setState({
          vehiclesToDisplay: results.data.vehicles
        })
    }).catch( () => {
      toast.error("Failed to up the price!")
    })
  }
  ...
  addCar() {
    ...
    axios.post('https://joes-autos.herokuapp.com/api/vehicles', newCar).then((results) => {
      toast.success("You've successfully added a new car!");
          this.setState({
            vehiclesToDisplay: results.data.vehicles
          })
    }).catch( () => {
      toast.error("Sorry, but you suck.")
    })
